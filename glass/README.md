...
Полностью самостоятельная современная версия Tutor Booking: **React + Node.js** и единый визуальный стиль liquid glass. Папка самодостаточна: в ней есть сервер, API, демо-хранилище, копия Google Apps Script, React-главная, React-кабинет ученика и стили для остальных страниц.
...
Вся бизнес-логика находится в `server.js`. Главная страница и кабинет ученика — React-приложения из `src/`, использующие прежние same-origin API-адреса и формат данных. Админка, Telegram и тест сохраняют проверенное JavaScript-поведение; `public/legacy-glass.css` приводит их интерфейс к общему стилю без рискованной переписи сложных сценариев.

## Независимая схема работы

```text
браузер
  ├── React-главная / React-кабинет
  ├── админка / Telegram / тесты
  └── те же адреса /api/…
                         ↓
...
Для быстрой работы с React-главной и React-кабинетом оставьте `npm start` на порту `3000` в одном терминале и запустите `npm run dev` во втором. Vite передаст относительные запросы `/api` в этот самостоятельный сервер. Полную проверку лучше делать через `npm start`, потому что тогда доступны также `/admin.html`, `/telegram.html` и `/test.html`.
