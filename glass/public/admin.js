...
(function () {
  ...
  function initTheme() {
    const btn = $("#adminThemeBtn");
    const saved = (() => { try { return localStorage.getItem("glass-theme") || localStorage.getItem("theme") || "dark"; } catch (e) { return "dark"; } })();
    document.body.classList.toggle("dark", saved !== "light");
    const paint = () => { if (btn) btn.textContent = document.body.classList.contains("dark") ? "☀️" : "🌙"; };
    paint();
    if (btn) btn.onclick = () => {
      document.body.classList.toggle("dark");
      const next = document.body.classList.contains("dark") ? "dark" : "light";
      try { localStorage.setItem("theme", next); localStorage.setItem("glass-theme", next); } catch (e) {}
      paint();
    };
  }
